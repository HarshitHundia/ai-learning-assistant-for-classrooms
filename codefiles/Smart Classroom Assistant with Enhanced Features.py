...
import pypdf # NEW: For reading PDF files
from docx import Document # NEW: For reading .docx files
...
GEMINI_API_KEY = "AIzaSyAgrpayn0eNJy-hqm8OrOOGf6mFrzyaGPE" # <--- YOUR API KEY HAS BEEN ADDED HERE!

# Debug print to verify API key is being picked up
if GEMINI_API_KEY:
    print(f"DEBUG: Using API Key (first 5 chars): {GEMINI_API_KEY[:5]}...")
else:
    print("DEBUG: API Key is empty. Gemini API calls will likely fail with 403 Forbidden.")
...
try:
    ...
    # In a production app, you might gracefully degrade or inform the user.
    exit()
...
# --- External Tool: Google Search Simulation ---
async def search_tool(query: str) -> str:
    """
    Simulates a real-time web search for the given query.
    In a live environment, this would integrate with a Google Search API
    or a similar external search service.
    """
    print(f"🔍 Performing simulated web search for: '{query}'...")
    try:
        # This is a simplified simulation for demonstration.
        # A real search API call would retrieve live data.

        # Simulate specific results for common queries
        if "Koppal Lok Sabha" in query or "MP Koppal" in query or "Member of Parliament Koppal" in query:
            return """
            Latest news (June 2024): K. Rajashekar Basavaraj Hitnal (Indian National Congress) won the Koppal Lok Sabha constituency in the 2024 General Elections. He defeated Basavaraj Kyavator (Bharatiya Janata Party).
            ...
            return """
            Latest news (May 2023 Karnataka Assembly Elections): G. Janardhana Reddy (Kalyana Rajya Pragathi Paksha) is the current MLA for Gangavati Assembly Constituency.
            ...
            return "The weather today in Bengaluru is partly cloudy with a high of 30°C and a low of 22°C. There's a 20% chance of rain."
        elif "prime minister of india" in query.lower():
            return "The current Prime Minister of India is Narendra Modi, who assumed office on May 26, 2014."
        else:
            # Generic fallback if no specific simulation matches
            print(f"No specific simulation for query: '{query}'. Returning generic message.")
            return f"No specific real-time data found for '{query}'. This is a simulated search result."
    except Exception as e:
        print(f"❌ Error in simulated search_tool: {e}")
        return f"Error in simulated search: {e}"
...
def transcribe_audio(audio_file_path):
    ...
    try:
        ...
        print("📝 You said (raw from Whisper):", raw_transcription)
...
async def main_assistant_loop():
    ...
    while True:
            ...
            if science_notes_content:
                ...
                if relevant_context:
                    ...
                    if "state that" in assistant_response.lower() or "not in the response" in assistant_response.lower() or "sorry" in assistant_response.lower() or "i cannot answer" in assistant_response.lower() or "could not find the answer" in assistant_response.lower():
                        ...
                        # In this case, get_gemini_answer already appends to chat_history
                        ...
                        # get_gemini_answer already appends both user and model roles. No extra append here.
                else:
        ...
        # Append the assistant's response to chat_history ONLY if it hasn't been added by get_gemini_answer
        # or if it's a direct command response.
        # The logic was slightly off; get_gemini_answer *already* adds both user and model responses.
        # For other commands, we explicitly added the model response inside their blocks.
        # This final check ensures we don't accidentally double-add, especially for tool calls
        # where the structure is different.
        # Simplified: if the last role added was 'user', and this path leads to a final assistant response, add it.
        # This is now handled by explicit `chat_history.append({"role": "model", "parts": [{"text": assistant_response}]})`
        # inside each command block that generates a final `assistant_response`.
        # No general `if chat_history and chat_history[-1]["role"] == "user":` needed here anymore.
